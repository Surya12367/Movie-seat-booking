...
const MovieSeatBooking = ({
  ...
  const getTotalPrice = () =>{
    return selectdSeats.reduce((total, seat) => total + seat.price, 0);
  };

  const handleBooking=()=>{
    if(selectdSeats.length === 0){
    ...
    setSeats((prevSeats)=>{
      return prevSeats.map((row)=>
        row.map((seat) =>{
        if (selectdSeats.some((selected) => selected.id === seat.id)) {
    ...
    onBookingComplete({
      ...
      seatIds: selectdSeats.map((seat)=> seat.id),
    });
  ...
  return (
    <div className='w-full min-h-sreen bg-gray-50 p-4'>
        {/* title */}
        ...
        <div className="flex flex-wrap justify-center gap-6 mb-6 p-4 bg-gray-50 rounded-lg">
          {uniqueSeatTypes.map((seatTypes) => {
            return <div className="flex items-center" key={seatTypes.type}>
              <div 
        ...
        {/* Summary */}
        <div className="bg-gray-50 rounded-lg p-4 mg-4">
          <h3 className='font-bold text-lg mb-2'>Booking Summary</h3>

          {selectdSeats.length>0?(
            <div>
              <p className="mb-2">
                Selected Seats:{" "}
                <span className="font-medium">
                  {selectdSeats.map((s) => s.id).join(", ")}
                </span>
              </p>
              <p className="mb-2">
                Number of Seats:{" "}
                <span className="font-medium">{selectdSeats.length}</span>
              </p>
        ...
        disabled={selectdSeats.length === 0} 
